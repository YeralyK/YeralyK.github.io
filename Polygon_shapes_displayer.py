...
# global constants
YOUR_ID = '124040030'   # TODO: your student id
...
def calculate_safe_area(poly_point):
    #Calculating the bounding box for a polygon based on its vertices
    
    if not poly_point:
        return ((0, 0), (0, 0))
    
    x_coord = [x for x, y in poly_point]
    y_coord = [y for x, y in poly_point]
    return ((min(x_coord), max(x_coord)),
            (min(y_coord), max(y_coord)))

def check_box_overlap(first_box, second_box):
    #Checks whether two bounding boxes overlap
    
    (min1_x, max1_x), (min1_y, max1_y) = first_box
    (min2_x, max2_x), (min2_y, max2_y) = second_box
    
    x_overlap = not (max1_x < min2_x or max2_x < min1_x)
    y_overlap = not (max1_y < min2_y or max2_y < min1_y)
    
    return x_overlap and y_overlap

def check_line_intersection(line1_start, line1_end, line2_start, line2_end):
    #Determining whether two line segments intersect
    
    def compute_orient(first, second, third):
        return (second[0] - first[0]) * (third[1] - first[1]) - (second[1] - first[1]) * (third[0] - first[0])

    o1 = compute_orient(line1_start, line2_start, line2_end)
    o2 = compute_orient(line1_end, line2_start, line2_end)
    if o1 * o2 >= 0:
        return False

    o3 = compute_orient(line2_start, line1_start, line1_end)
    o4 = compute_orient(line2_end, line1_start, line1_end)
    return o3 * o4 < 0

def is_point_in_polygon(point, poly_point):
    #Determines whether a given point is inside a polygon (ray-casting)
    
    px, py = point
    point_count = len(poly_point)
    inside_flag = False

    for i in range(point_count):
        start_vert = poly_point[i]
        end_vert = poly_point[(i + 1) % point_count]
        x1, y1 = start_vert
        x2, y2 = end_vert

        if (px, py) in (start_vert, end_vert):
            return True
...
def is_shape_overlapped_any(shape: turtle.Turtle, shapes: list[turtle.Turtle]) -> bool:
	...
	'''
    if not g_shape_coords:
        with open(SHAPE_FILE, 'r') as f:
            for line in f:
                if ':' not in line:
                    continue
                name, coords = line.split(':')
                g_shape_coords[name.strip()] = eval(coords.strip())

    shape_type = shape.shape()
    shape_x, shape_y = shape.position()
    shape_blueprint = g_shape_coords.get(shape_type, [])

    shape_vertix = [(shape_x + dx, shape_y + dy) for (dx, dy) in shape_blueprint]

    if not shape_vertix:
        return False

    def padded_box(box, pad = XY_STEP + 5):
        (min_x, max_x), (min_y, max_y) = box
        return ((min_x - pad, max_x + pad), (min_y - pad, max_y + pad))

    new_bounding_box = padded_box(calculate_safe_area(shape_vertix))

    for exist_shape in shapes:
        exist_type = exist_shape.shape()
        exist_x, exist_y = exist_shape.position()
        exist_blueprint = g_shape_coords.get(exist_type, [])
        exist_vertix = [(exist_x + dx, exist_y + dy) for (dx, dy) in exist_blueprint]

        if not exist_vertix:
            continue

        existing_box = calculate_safe_area(exist_vertix)

        if not check_box_overlap(new_bounding_box, existing_box):
            continue

        for pt in shape_vertix:
            if is_point_in_polygon(pt, exist_vertix):
                return True
        for pt in exist_vertix:
            if is_point_in_polygon(pt, shape_vertix):
                return True
            
        for i in range(len(shape_vertix)):
            a1 = shape_vertix[i]
            a2 = shape_vertix[(i + 1) % len(shape_vertix)]
            for j in range(len(exist_vertix)):
                b1 = exist_vertix[j]
                b2 = exist_vertix[(j + 1) % len(exist_vertix)]
                if check_line_intersection(a1, a2, b1, b2):
                    return True
